...
void updateAllWords(std::vector<Word>& words, GameState& gameState, float deltaTime) {
    for (auto it = words.begin(); it != words.end();) {
        updateWordPosition(*it, deltaTime);
        
        // Eliminar palabras que llegaron al fondo
        if (isWordOutOfBounds(*it, 1400)) {
            if (!it->isActive || !isWordCompleted(*it)) {
...
void removeCompletedWords(std::vector<Word>& words) {
    words.erase(
        std::remove_if(words.begin(), words.end(), 
            [](const Word& word) { return isWordCompleted(word); }),
        words.end()
    );
}
...
void resetGame(WordSpawner& spawner, GameState& gameState, InputHandler& inputHandler, 
    ...
    resetSpawnerSettings(spawner);
    restartGame(gameState);
    clearInputBuffer(inputHandler);
    activeWords.clear();
}
...
int main() {
    ...
    // Crear instancias de las clases usando funciones estructuradas
    WordSpawner spawner = createWordSpawner();
    GameState gameState = createGameState();
    Renderer renderer = createRenderer(&window);
    InputHandler inputHandler = createInputHandler();
    
    // Cargar assets del renderer
    if (!initializeAssets(renderer)) {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            switch (currentGameState) {
                ...
                case PLAYING:
                    ...
                    processKeyboardInput(inputHandler, event, gameState, activeWords);
        ...
        switch (currentGameState) {
            ...
            case PLAYING:
                ...
                if (checkSpawnTime(spawner)) {
                    Word newWord = spawnRandomWord(spawner);
                ...
                checkWordMatches(inputHandler, activeWords, gameState);
                
                // Eliminar palabras completadas
                removeCompletedWords(activeWords);
                
                // Renderizar frame del juego
                renderFullFrame(renderer, activeWords, gameState);
                break;
